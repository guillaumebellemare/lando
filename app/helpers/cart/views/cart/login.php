...
<div class="l-grid l-row-2">
  <div class="l-grid-50">
    ...
    <div class="cart-card">
      <form action="<?=URL_ROOT . $this->lang2 . "/" . $routes["checkout_login"]?>" method="post" name="fATC">
    ...
    <br>
    <p><a href="<?=URL_ROOT.$this->lang2."/".$routes["forgot"]?>"><?=$login["forgot-password"]?></a></p>
  </div>
  <div class="l-grid-50">
    <h1><?=$login["new_user"]?></h1>
    <div class="cart-card">
      <p><?=$login["new_user_text"]?></p>
      <a href="<?=URL_ROOT . $this->lang2 . "/" . $this->routes["checkout_billing-address"]?>" class="btn is-full-width"><?=$login["continue_invited"]?></a>
    </div>
  </div>
</div>
<?php endblock() ?>
