...
*/
	...
	protected function __getDestinationWODuplicate($file, $target_folder){
		$file_name = basename($file);
		if(file_exists($target_folder . '/' . $file_name)){
			$count = 0;
			$dotPos = strrpos($file_name, '.');
			$real_name = substr($file_name, 0, $dotPos);
			$ext = substr($file_name, $dotPos+1);
			
			do{
				$file_name = $real_name . '_' . $count . '.' . $ext;
				$count++;
			}while(file_exists($target_folder . '/' . $file_name));
		}
    ...
    protected function get_unique_filename($file_path, $name, $size, $type, $error,
            $index, $content_range) {
            $folder = dirname($this->get_upload_path($name));
            $name = basename($this->__getDestinationWODuplicate($name, $folder));
       /*
