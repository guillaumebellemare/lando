...
while ($currentRoute = current($routes)) {
	...
	if ($page == $currentRoute && !$controller_loaded) {
		...
		if(file_exists('app/controllers/'.ucfirst(key($app_routes)).'Controller.php'))
		{
			...
			if($debug==true && $debug_on==true) $app_messages[] = '<strong>Current controller: </strong>'.$current_controller.'@'.$current_function.'<br>';
		...
		{
			if($debug==true && $debug_on==true) $app_messages[] = '<strong>Current view:</strong> app/views/'.$current_route.'/'.$current_function.'.php';
...
// Error 404
if($page_setted==0) {
	if($debug==true && $debug_on==true) $app_messages[] = '<strong>Current view:</strong> app/views/404/index.php';
